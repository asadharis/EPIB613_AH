...
p.val <- 2 * pt(q = t, df = n-1, lower.tail = F); p.val # Rejected at alpha=0.05.
...
p.val <- 2 * pnorm(q = z, mean = 0, sd = 1); p.val # Rejected at alpha=0.05.
...
t.test(y1, y2, mu = 0.5, alternative = "greater")   # Default is "two-sided".
