...
fit1 <- glm(admit~rank, family = binomial(link = "logit"), data = df); summary(fit1)
...
fit2 <- glm(admit~gpa, family = binomial(), data = df); summary(fit2)
...
fitted.admit <- predict(fit2, newdata = data.frame(gpa = seq(lb,ub,0.01)),
                        type = "response")
...
fit4 <- glm(incidents~type, family = poisson(link = "log"), data = ds)
summary(fit4)
...
fit5 <- glm(incidents~type+offset(log(service)), family = poisson(), data = ds); summary(fit5)
